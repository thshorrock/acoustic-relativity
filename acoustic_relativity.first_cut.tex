...
% The vector potential $A$ may be interpreted as an enthalpy current by using an  argument developed by Taub\cite{Taub1978}.
% From the isentropic thermodynamic relation $m de = - p d\lr{\frac{1}{n}}$ and the relation for the internal energy, $\epsilon= nm( 1 + e(p))$,
% it follows that 
% \begin{align}
%  n d\epsilon = \epsilon dn - n^2 p d \lr{\frac{1}{n}} = \lr{\epsilon + p} dn,
% \end{align}
% where $n$ is the particle number density of the medium and $m$ is the mass per particle.
% Applying equation \eqnref{eos} and integrating we obtain
% \begin{align}
...
% \begin{align}
%   \phi =    \frac{\epsilon + p }{n} = m\lr{c^2 + h},
% \end{align}
% where $h = e + p/(nm)$ is the  enthalpy.
% which is the heat function per particle.
% In the  non-relativistic limit this becomes
% \begin{align}
%  \phi \rightarrow  mc^2 + m \frac{\epsilon + p }{n}  = m,
% \end{align}
%\begin{align}
...
% To demonstrate this, we first note that the relativistic generalisation to the velocity potential, $\psi$,  is defined by\cite{LandauBook}
% \begin{align}
%   \d_i \psi = - \frac{\epsilon+p}{n} u_i = A_i
% % \del \psi = - \frac{\epsilon+p}{n} v = - \frac{2\epsilon}{n} v,
% \end{align}
% where $n$ is the proper particle number density of the fluid and equation \eqnref{eos} has been used for the second equality.
