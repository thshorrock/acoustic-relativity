...
This elmination of the \aether, however, only emphasises the uniqueness of light, for it is the  only  signal that has a  medium with no measurable mechanical properties.
...
A physicist moving with respect to the apparatus of \figref{setupA} %that is stationary with respect to the medium
%attempting to infer the passage of the sound through \figref{SetupB} %the apparatus that moves with respect to the bulk 
% equipment %(unlike the Galilean observer they cannot directly measure it)
 will  draw the same figure, \figref{MovingGA}, as the Galilean observer.
...
The solution is that the error introduced by setting the effective speed of sound to equal $c$  must be compensated by rescaling the temporal units used in ultrasound.
...
\end{align}
When a measurement is not stationary with respect to the medium the acoustically measured distance must be
contracted by the Lorentz factor in order to agree with experimental results.
This is the contraction that is postulated by \Poncare's special relativity.
The contraction exists because any  signal that is used to measure distances must be assumed to be constant,
not because the signal is constant, but because distances cannot be measured otherwise.
\Poincare's contraction is true because it is experimentally measured.
There is underlying physical reason for the contraction: it exists due to variations in the sound speed that are impossible to measure.
...
It is instructive to carry out this calculation,
and to reformulate Lighthill's theory of aeroacoustics.
