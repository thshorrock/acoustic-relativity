...
The answer, for Einstein, is  that light has a constant speed that  is independent of the motion of the light source.
...
The measurement is local to the observer and all they require is a local light source and receiver. 
...
`Stationary' means stationary with respect to the \aether\ and
...
For \Poincare\ a rod that moves with respect  to the \aether\ really is contracted in comparison to  its length when stationary.
The contraction is determined from the Lorentz formula.
For \Poincare\ this is a postulate.
The second postulate used by \Poincare\ is that there is no absolute reference from  which to measure the speed of the \aether.
Using these two postulates \Poincare\ derived the temporal Lorentz transformation:
...
Indeed in 1900 \Poincare\ writes
\begin{quote}
...
Light is not privileged in  \Poincare's theory but at the cost of  introducing motion dependant deformations.
...
It may be measured by any observer by translating the rod so that it becomes stationary with respect to that observer.
When the rod moves with respect to an observer it {\em appears} contracted according to the Lorentz formula.
However, it is actually the same {\em rigid} rod and can be measured to have the same length by an observer that moves with the rod.
When faced with a choice between the relativity theories of Einstein and \Poincare\ the community choose Einstein's. % because it appeared more complete.
If, after all, light really is different from any other signal then Einstein's theory is complete.
...
It is seen in \secref{measurement} that \Poincare's motion dependent contraction is understood  in this context by taking it very literally.
...
We must admit, however, that  the measured dimensions of an object that moves with respect to its medium  does  depend upon whether
...
However, the speed of sound is here a constant not because it is `true'
but because there is no other choice but to assume its constancy.
...
It follows that the acoustic medium must be incompressible (in the relativistic sense)
...
with the world that is measured with ultrasound. % - on a simple pulse echo experiment.
...
They can directly measure this with the definitions of equation~\eqnref{radar}.
...
and so the ultrasound physicist predicts that sound propagates at a  constant speed, $c$.
%- 
...
The ultrasound physicist cannot measure the sound speed and so  must work a little harder to come to this conclusion.
...
However, this explanation can be eliminated by counting  the number of sound pulses that propagate through each path.
Counting the number of pulses that fit within the apparatus is straight forward with ultrasound.
The pulse length is simply increased until the  received signal starts to interfere with the  emitted signal.
The result is that $n = 2l/\lr{cT}$ cycles fit both  within the path  $A$-$B$-$A^\prime$  and $A$-$C$-$A^\prime$, where $T$ is the period of the sound wave.
...
That is parallel to the flow the {\em   unit of distance} is shorter  by a factor of $\gamma$.
...
This result can occur  when either the  frequency of the ultrasound pulse is shifted by a factor of $\gamma$
or when the {\em  unit of time} is scaled  by the factor of $\gamma$.
The first of these interpretations does not make sense physically because the frequency of the pulse is a function of the transducer and driving pulse
and not the transducer's motion.
...
When modelling the results of an acoustic experiment,
where the acoustic source is not stationary with respect to the medium,
the modelled unit of distance must be
contracted by the Lorentz factor in order to agree with experimental results,
and likewise the modelled unit of time must be slow by the Lorentz factor.
%The   contraction in length is postulated by \Poincare's special relativity.
The contraction in length exists because a  signal that is used to measure distances must be assumed to be a constant,
...
It is instructive to formulate acoustics for  a  medium that is measured with ultrasound.
First it is shown in \secref{MaxwellAnalogue} that the acoustics when measured with sound obeys Maxwell's relation,
and then it is shown in \secref{int:EM} that this formulation is essentially the same as  Lighthill's theory of aeroacoustics.
...
The equations of motion must  be Lorentz invariant and this condition  is automatically fulfilled  when  the equations of motion
are obtained from the divergence of the energy-momentum tensor of the ideal fluid.
Remembering that the sound speed takes the role  of the speed of light
we equate these two speeds.
...
$p$ is the pressure
and 
$u=u(\tau)$ is the velocity vector of the spacetime path, with the parameterisation chosen such that $u^2 = u^i u_i = 1$. % and $P$ is the pressure.
...
\end{align}
where $\d_j \equiv \frac{\d}{\d x^j}$,  $n$ is the proper particle number density of the fluid.
Equation \eqnref{eos} has been used for the second equality.
To show that this is equal to the negative of the potential $A$, we use an argument of Taub\cite{Taub1978}.
The internal energy, $\epsilon$, is equal to the sum of the rest mass and the thermodynamic internal energy, $e$ \cite{LandauBook, Taub1978},
\begin{align}
  \epsilon(p) = nm( 1 + e(p)),
\end{align}
...
Equation \eqnref{wave} is a wave equation and so we interpret the right-hand-side of \eqnref{wave} as an acoustic  current source, $J$.
Therefore 
\sub{
\label{eqn:Maxwell}
\eqa{
...
which follows due to the use of the repeated differential and the Levi-Civita permutation tensor, $\epsilon_{i j k l}$.
...
To demonstrate the similarities and the differences of the ultrasound formulation of acoustics  to the Gallilean formulation,
...
Equation~\ref{eqn:phinr} multiplies this by the particle mass, $m$, and adds the rest energy $mc^2$ absent from all non-relativistic thermodynamics.
%\begin{align}
...
The difference between the acoustically measured continuity equation, \label{eqn:NRcontinuity}, 
...
The vector $\vE$ is known as the Lamb vector and is proportional to the Coriolis acceleration, it takes the role of the electric field in the analogy.
...
This report has demonstrated that when acoustic definitions of time and space are used to measure the world
...
Ultrasound, as well as other sonar-based technologies, is in this sense  a relativistic subject
where  the  speed of sound takes the traditional role of the speed of light.

Light signals have no special function in the relativity theory used in this report 
and are here replaced by acoustic signals.
The propagating signal traverses at a constant speed, not because it really does have a constant speed, 
but because it must be assumed to have a constant speed in order to measure distances with local temporal measurements.
...
It was then demonstrated that the acoustics of the medium obey Maxwell's relation.
It was shown that Gauge invariance in Maxwell's relation may be interpreted as an invariance to the  potential flow of the medium.
...
However, this is at odds with the current understanding in medical ultrasound that non-linear propagation of an acoustic pulse
