...
Therefore, in ultrasound theory, fluctuations in the density  cannot alter the sound speed. %can therefore play no role in determining the sound speed. % when ultrasound is used to describe the world.
...
Equation~\eqnref{NREuler} is the incompressible version of Euler's equation written in Crocco's form\cite{Howe1998}
...
\subl{
  \begin{align}
    \lrsquare{  \Dt \lr{\frac{1}{c^2} \Dt} - \frac{1}{\rho}\vdel \cdot \lr{\rho \vdel}}\frac{\phi}{m} &= -\frac{1}{\rho} \vdel \cdot \lr{\rho \vv \times \lr{\vdel\times \vv}} \label{eqn:WavephiREG}\\
...
In this report it has been demonstrated that when sound is used to measure distance - as it is in sonar-based technologies -
...
This is so because to measure distances from the to-fro propagation time of a signal, 
...
All that can be said is that measurements that are faster than the  propagating signal cannot be predicted.
...
It is seen that the only major difference between the two is the term $- \frac{1}{c^2}\dt \lr{ \vv \times \lr{\vdel \times \vA}}$ on the right-hand-side of \eqnref{WavevA}.
...
the term $- \frac{1}{c^2} \dt \lr{ \vv \times \lr{\vdel \times \vA}}$ most certainly is a current.
...
The acoustic analogue to special relativity presented here is {\em identical} to Poincare's relativity,
